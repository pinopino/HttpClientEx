...
namespace HttpClientEx
{
    public static class HttpClientExtensions
    {
        ...
        /// <param name="headers">自定义请求头</param>
        /// <returns>返回的HttpResponseMessage</returns>
        public static async Task<HttpResponseMessage> PutAsync(this HttpClient httpClient, string url,
            object body, QueryString queryString = null, IDictionary<string, string> headers = null)
        {
            if (string.IsNullOrEmpty(url))
                throw new ArgumentNullException(nameof(url));
        ...
        {
            var finalUrl = UrlUtility.Combine(httpClient.BaseAddress.OriginalString, url);
            if (queryString == null)
